...
Evaluates the [incomplete beta function][incomplete-beta-function].

```c
double y = stdlib_base_betainc( 0.5, 2.0, 2.0, 1, 0 );
// returns 0.5

y = stdlib_base_betainc( 0.5, 2.0, 2.0, 0, 0 );
// returns ~0.083
```
...
-   **a**: `[in] double` first function parameter.
-   **b**: `[in] double` second function parameter.
-   **regularized**: `[in] int` boolean indicating if function should evaluate regularized incomplete beta function.
-   **upper**: `[in] int` boolean indicating if function should return upper tail of incomplete beta function.
...
int main( void ) {
    ...
    double b[] = { 2.0, 2.0, 1.0 };
    double y;
    int i;

    for ( i = 0; i < 3; i++ ) {
        y = stdlib_base_betainc( x[i], a[i], b[i], 1, 0 );
